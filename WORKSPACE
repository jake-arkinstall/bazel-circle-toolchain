...
workspace(
    name = "bazel_circle_toolchain",
)

load("@bazel_circle_toolchain//toolchain:deps.bzl", "bazel_toolchain_dependencies")

bazel_toolchain_dependencies()

load("@bazel_circle_toolchain//toolchain:rules.bzl", "circle_toolchain")

circle_toolchain(
    name = "circle_toolchain",
    circle_version = "142",
)

load("@circle_toolchain//:toolchains.bzl", "circle_register_toolchains")

circle_register_toolchains()
...
circle_toolchain(
    name = "circle_toolchain_with_sysroot",
    circle_version = "142",
    sysroot = {
        "linux-x86_64": "@org_chromium_sysroot_linux_x64//:sysroot",
    },
    # We can share the downloaded Circle distribution with the first configuration.
    toolchain_roots = {
        "": "@circle_toolchain_circle//",
    },
)
