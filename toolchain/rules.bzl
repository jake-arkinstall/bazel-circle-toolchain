...
load(
    ...
    _circle_config_impl = "circle_config_impl",
)
load(
    "//toolchain/internal:repo.bzl",
    _circle_repo_impl = "circle_repo_impl",
)

_common_attrs = {
    "circle_version": attr.string(
        mandatory = True,
        doc = "One of the supported versions of LLVM, e.g. 12.0.0",
    ),
}

_circle_repo_attrs = dict(_common_attrs)
_circle_repo_attrs.update({
    "_circle_release_name": attr.label(
        default = "//toolchain/tools:circle_release_name.py",
        allow_single_file = True,
        doc = "Python module to output LLVM release name for the current OS.",
    ),
})

_circle_config_attrs = dict(_common_attrs)
_circle_config_attrs.update({
    "toolchain_roots": attr.string_dict(
        ...
        doc = ("System or package path, for each host OS and arch pair you want to support " +
               ...
               "specify a fallback default for all hosts, e.g. with the circle_toolchain_repo rule. " +
               ...
               "filegroup targets as in BUILD.circle_repo."),
    ),
...
circle = repository_rule(
    attrs = _circle_repo_attrs,
    local = False,
    implementation = _circle_repo_impl,
)

toolchain = repository_rule(
    attrs = _circle_config_attrs,
    local = True,
    configure = True,
    implementation = _circle_config_impl,
)

def circle_toolchain(name, **kwargs):
    if not kwargs.get("toolchain_roots"):
        circle_args = {
            k: v
            for k, v in kwargs.items()
            if (k not in _circle_config_attrs.keys()) or (k in _common_attrs.keys())
        }
        circle(name = name + "_circle", **circle_args)
        kwargs.update(toolchain_roots = {"": "@%s_circle//" % name})

    toolchain_args = {
        k: v
        for k, v in kwargs.items()
        if (k not in _circle_repo_attrs.keys()) or (k in _common_attrs.keys())
    }
